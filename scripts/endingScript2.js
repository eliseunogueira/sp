...
var focused,
  numOfConj,
  slidesPres = [];
numOfConj = 'French' != lessonLanguage ? 7 : 6;
for (
  var conjAlt,
    conjAltNum,
    conjAlt2,
    ...
    (screenRatio >= 1.5
      ...
      3 == currentQuiz &&
        ...
        screenRatio > 1
          ? ($('#sketchImg').show(),
            (theFont =
              viewportHeight > 600 ? 28 : viewportHeight < 450 ? 22 : 25),
            viewportHeight > 450
            ...
            $('#noteBook').css({ height: 1.03 * t + 'px' }),
            $('#writeAnswerBoard').css({ height: 0.4 * spaceLeft + 'px' }),
          ...
          : ($('#hintTable').css({ left: '3%', width: '94%' }),
            ...
            $('#quizAllItemsC').css({
              'margin-left': '6%',
              'padding-top': 0.05 * spaceLeft + 'px',
            }),
            $('#noteBook').css({ height: 1 * t + 'px' }),
            ...
            $('.quizSentenceWrap').css({
              'margin-top': 0.05 * spaceLeft + 'px',
            }),
            $('#quizWriteImg').css({ height: 0.54 * spaceLeft + 'px' }),
            $('#quizAllItemsC').css({ height: 0.38 * spaceLeft + 'px' }),
            $('#writeAnswerBoard').css({
              height: 0.38 * spaceLeft - 70 + 'px',
            }))),
      setTimeout(function () {
...
function resizeButtonQuizText() {
  for (var e = 1; e <= 6; e++) {
    ...
    for (
      var t = $('.textWrap5').width(), i = $('#quizEng' + e).width(), s = 13;
      t > i && s > 0.3;
    )
      ((s -= 0.5),
        $('.textWrap5').css({ fontSize: s + 'px' }),
        $('#quizEng' + e).css({ fontSize: s + 'px' }),
        (t = $('.textWrap5').width()),
        (i = $('#quizEng' + e).width()));
  }
  for (e = 1; e <= 4; e++) {
    ...
    for (
      var n = $('#quizMultiImg').width() - 20,
        o = $('#quizItem' + e + 'B').width(),
        r = 22;
      o > n && r > 0.3;
    )
      ((r -= 0.5),
        $('#quizItem' + e + 'B').css({ fontSize: r + 'px' }),
        (o = $('#quizItem' + e + 'B').width()));
  }
}
function resizePage() {
  ...
  if (
    ...
    for (
      var e = $('.menuBtnContainer').width(),
        ...
        i = $('.smallLogo').width(),
        s = viewportWidth - (i + t + e);
      s < 60 && $('.menuTitleContainer').text().length > 6;
    ) {
...
function changeWord() {
  ...
  if (currentWordNum < narration.length) {
    ...
    for (var s = 1; s <= numOfConj; s++)
      $('#sideEnding' + s).html(theEndings[s - 1]);
    ...
    for (
      var n = 1;
      n <= window['forConj' + i + verbLetter[verbInList - 1]].length;
      n++
    )
      (1 == n && 'French' == lessonLanguage
        ? -1 != vowels.indexOf(replaceText[verbInList - 1].charAt(0))
          ? $('#conjItem' + n).append(
              window['forConj' + i + verbLetter[verbInList - 1]][n - 1]
        ...
        : $('#conjItem' + n).append(
            window['forConj' + i + verbLetter[verbInList - 1]][n - 1].replace(
              'zzzz',
    ...
    if (($('.narrSentence').hide(), 0 == t)) {
      if ('picPrev' != picControl) {
        (e < numOfPers
          ...
          currentWordNum < narration.length + numOfPers
            ? ((verbType = verbTypes[0]), (theEndings = passEndings1))
            : currentWordNum < narration.length + 2 * numOfPers
              ? ((verbType = verbTypes[1]), (theEndings = passEndings2))
              : currentWordNum < narration.length + 3 * numOfPers &&
                ((verbType = verbTypes[2]), (theEndings = passEndings3)),
          $('#sideTitle').html(
            '<strong>' +
              verbType +
              '</strong> Endings (' +
        ...
        for (n = 1; n <= window['forConj' + i].length; n++)
          (1 == n && 'French' == lessonLanguage
            ...
            : $('#conjItem' + n).append(
                window['forConj' + i + verbLetter[verbInList - 1]][
                  n - 1
                ].replace('zzzz', replaceText[verbInList - 1]),
              ),
            $('#conjItemB' + n).append(
      ...
      ($('.conjItems').removeClass('hilite'),
        ...
        $('#sideHand' + t).addClass('sideHandsAnimate'),
        $('.forSentence').html(
          window['forConj' + i + verbLetter[verbInList - 1]][t - 1]
            .replace('zzzz', replaceText[verbInList - 1])
            .replace('(', "<b style='opacity:0.5;'>(")
            .replace(')', ')</b>'),
        ),
...
function resizeText() {
  ...
  for (
    ...
    e = $('.textWrapB').width();
    var n = s;
    $('.forSentenceB').css({ fontSize: n + 'px' });
  }
  ...
  for (
    ...
    e = $('.textWrap').width();
    n = s;
    ($('.forSentence').css({ fontSize: n + 'px' }),
      s < 20 && $('.forSentence').addClass('smallFont'));
  }
...
function resizeQuizText() {
  var e, t, i;
  ($('#quizSentenceFor').css({ fontSize: '28px' }),
    (e = $('#quizSentenceFor').width()),
    (t = $('#quizContent').width() - 15));
  for (var s = 28; e > t; s--)
    ((i = s),
      $('#quizSentenceFor').css({ fontSize: i + 'px' }),
      (e = $('#quizSentenceFor').width()));
  ($('#quizSentenceEng').css({ fontSize: '18px' }),
    (e = $('#quizSentenceEng').width()),
    (t = $('#quizContent').width() - 15));
  for (s = 18; e > t; s--)
    ((i = s),
      $('#quizSentenceEng').css({ fontSize: i + 'px' }),
      (e = $('#quizSentenceEng').width()));
  if (2 == currentQuiz) {
...
function loadQuiz3() {
  ...
  var t = shuffle([1, 2, 3, 4, 5, 6]),
    ...
    s = shuffle([13, 14, 15, 16, 17, 18]);
  ((quizCombinedArray = t.concat(i.concat(s))),
    $('#wrongAnswerIn').css({
      'background-image': 'url(../../common/quizWrong3.svg)',
    }),
    findAccents(),
    ...
    1 == currQuizNum
      ...
      : 7 == currQuizNum
        ...
        : 13 == currQuizNum
          ...
          : ((timerWait = 500), quizTimerAnimate()));
}
...
function quizActions3() {
  ...
  for (var i = 0; i < articleArray.length; i++)
    0 == writtenString.indexOf(articleArray[i]) &&
      (foundArticle = articleArray[i]);
  ('' != foundArticle
    ...
    (timerTime = 5e3 + 500 * allLetters.length));
  for (i = 0; i < allChars.length; i++)
    -1 != quizGoodPunct.indexOf(allChars[i]) && (timerTime += 1e3);
  letterArray = [];
  for (i = 0; i < allLetters.length; i++) letterArray.push(i);
  ...
  for (i = 0; i < scrambled.length; i++) {
    var o = scrambled[i].split('');
    scrambled[i] = shuffle(o);
  }
...
function addCharacter() {
  var e = $('#writeAnswerField').caret(),
    t = $(this).text(),
    i = $('#writeAnswerField').val();
  ((i = [i.slice(0, e), t, i.slice(e)].join('')),
    $('#writeAnswerField').val(i),
    (focused = !0),
...
function checkWrittenAnswer() {
  if ('' != $('#writeAnswerField').val()) {
    for (
      var e = quizGoodPunct.toLowerCase().split(' ').join(''),
        t = removeAccent(e),
        i = $('#writeAnswerField')
          .val()
          .toLowerCase()
      ...
      e.charAt(r) != i.charAt(r) && n.push(r);
    if (i.length <= o - 1) {
      var a = n[0],
        l = e.slice(a);
      ((m = 0 != a ? i.slice(a + -1) : '%' + i), (n = []));
      for (r = 0; r < l.length; r++)
        l.charAt(r) != m.charAt(r) && n.push(r + a);
    }
    if (i.length == o - 2) {
      var c,
        d = n[1],
        u = e.slice(d);
      ((c = 0 != d ? i.slice(d - 2) : '%' + i), (n = n.slice(0, 1)));
      for (r = 0; r < l.length; r++)
        u.charAt(r) != c.charAt(r) && n.push(r + d);
    }
    if (i.length == o + 1) {
      var m;
      ((a = n[0]), (l = e.slice(a)));
...
function startLesson() {
  ...
  for (var r = 1; r <= numOfConj; r++)
    ($('#sideEnding' + r).html(allEndings[r - 1]),
...
function shuffle(e) {
  for (
    var t, i, s = e.length;
    s;
    t = Math.floor(Math.random() * s), i = e[--s], e[s] = e[t], e[t] = i
  );
  return e;
}
...
((window.onload = function () {
  ...
  $(document).ready(function () {
    ((viewport = document.querySelector('meta[name=viewport]')),
      ...
      -1 != osType.indexOf('droid') &&
        ($('#musCol').hide(), $('#menuPad').css({ height: '225px' })),
      ...
      'French' == lessonLanguage
        ...
        : 'Italian' == lessonLanguage
          ...
          : 'Spanish' == lessonLanguage && $('#starConjSame').hide(),
      7 == conjNum &&
        $(
          '.starQuestion, .starQuestion2, #pronounWarn, #starConjWarn, .lineJoiner',
        ).hide(),
      ...
      (1 == verbTypeNum || 2 == verbTypeNum || 3 == verbTypeNum) &&
...
'Italian' == lessonLanguage
  ...
  : 'Spanish' == lessonLanguage
    ...
    : 'Portuguese' == lessonLanguage
      ...
      : 'French' == lessonLanguage && (verbTypes = ['ER', 'IR', 'RE']);
  ...
  return function (n, o, r) {
    ...
    var l = r ? 'getUTC' : 'get',
      ...
      b = {
        ...
        o:
          (z > 0 ? '-' : '+') +
          s(100 * Math.floor(Math.abs(z) / 60) + (Math.abs(z) % 60), 4),
        S: ['th', 'st', 'nd', 'rd'][
